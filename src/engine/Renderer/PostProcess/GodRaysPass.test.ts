...
//   7. dispose() 释放资源
//   8. setDirty() 触发重建
//   9. samples 字段可更新且被 clamp
//  10. GOD_RAYS_FRAG shader 源码校验

import { describe, it, expect } from 'vitest';
import { GodRaysPass } from './GodRaysPass';
import { PerspectiveCamera } from '../../Cameras/PerspectiveCamera';
import { GOD_RAYS_FRAG } from '../../Materials/shaders';

// ── MockGL2(复用 WaterSurfacePass.test.ts 模式,含 RGBA16F/HALF_FLOAT) ──
...
describe('GodRaysPass construction', () => {
  ...
  it('samples is updatable', () => {
    const p = new GodRaysPass();
    expect(p.samples).toBe(80);
    p.samples = 160;
    expect(p.samples).toBe(160);
  });
});

// ── apply / 资源生命周期 ──────────────────────────────────────────

describe('GodRaysPass apply', () => {
  ...
  it('disabled returns input texture with zero draw calls', () => {
    const gl = new MockGL2();
    const p = new GodRaysPass({ enabled: false });
  ...
  it('handles large samples value (clamped to 256 in apply)', () => {
    const gl = new MockGL2();
    const p = new GodRaysPass({ samples: 512 });
...
describe('GodRaysPass dispose / setDirty', () => {
...
describe('GOD_RAYS_FRAG shader source', () => {
  it('is GLSL ES 3.0', () => {
    expect(GOD_RAYS_FRAG).toContain('#version 300 es');
  });

  it('declares required uniforms', () => {
    expect(GOD_RAYS_FRAG).toContain('u_colorMap');
    expect(GOD_RAYS_FRAG).toContain('u_depthMap');
    expect(GOD_RAYS_FRAG).toContain('u_viewProjection');
    expect(GOD_RAYS_FRAG).toContain('u_lightPosition');
    expect(GOD_RAYS_FRAG).toContain('u_lightColor');
    expect(GOD_RAYS_FRAG).toContain('u_lightIntensity');
    expect(GOD_RAYS_FRAG).toContain('u_samples');
    expect(GOD_RAYS_FRAG).toContain('u_decay');
    expect(GOD_RAYS_FRAG).toContain('u_exposure');
    expect(GOD_RAYS_FRAG).toContain('u_density');
    expect(GOD_RAYS_FRAG).toContain('u_threshold');
    expect(GOD_RAYS_FRAG).toContain('u_maxDepth');
    expect(GOD_RAYS_FRAG).toContain('u_enabled');
  });

  it('projects light world position to screen via viewProjection', () => {
    expect(GOD_RAYS_FRAG).toContain('u_viewProjection * vec4(u_lightPosition');
    expect(GOD_RAYS_FRAG).toContain('lightClip.w');
  });

  it('skips when light is behind camera (clip w <= 0)', () => {
    expect(GOD_RAYS_FRAG).toContain('lightClip.w <= 0.0');
  });

  it('computes radial sampling delta from pixel to light', () => {
    expect(GOD_RAYS_FRAG).toContain('lightScreenUV - v_uv');
    expect(GOD_RAYS_FRAG).toContain('deltaTexCoord');
  });

  it('uses bounded loop with MAX_SAMPLES=256', () => {
    expect(GOD_RAYS_FRAG).toContain('MAX_SAMPLES = 256');
    expect(GOD_RAYS_FRAG).toContain('if (i >= u_samples) break');
  });

  it('extracts brightness via luminance threshold', () => {
    expect(GOD_RAYS_FRAG).toContain('luminance');
    expect(GOD_RAYS_FRAG).toContain('lightMask');
    expect(GOD_RAYS_FRAG).toContain('u_threshold');
  });

  it('applies depth-based occlusion (sky passes, geometry blocks)', () => {
    expect(GOD_RAYS_FRAG).toContain('occlusion');
    expect(GOD_RAYS_FRAG).toContain('step(u_maxDepth, sampleDepth)');
  });

  it('accumulates with exponential illumination decay', () => {
    expect(GOD_RAYS_FRAG).toContain('illuminationDecay');
    expect(GOD_RAYS_FRAG).toContain('illuminationDecay *= u_decay');
  });

  it('uses additive compositing (sceneColor + rays)', () => {
    expect(GOD_RAYS_FRAG).toContain('sceneColor + rays');
  });

  it('handles disabled state (u_enabled == 0)', () => {
    expect(GOD_RAYS_FRAG).toContain('u_enabled == 0');
  });

  it('references GPU Gems 3 and o3de', () => {
    expect(GOD_RAYS_FRAG).toContain('GPU Gems 3');
    expect(GOD_RAYS_FRAG).toContain('o3de');
  });
});

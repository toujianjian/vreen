...
// ── MockGL2 ─────────────────────────────────────────────────────────
// 支持 GTAOPass.apply 所需的全部 GL 调用表面,包括 ShaderProgram 编译路径。
// GTAOPass 输出 RGBA8 + UNSIGNED_BYTE,因此需要相应常量。
...
describe('GTAOPass construction', () => {
  it('defaults: radius=0.5, thickness=0.2, power=1.5, maxPixels=32, enabled=true', () => {
    const p = new GTAOPass();
    expect(p.name).toBe('gtao');
    expect(p.radius).toBe(0.5);
    expect(p.thickness).toBe(0.2);
    expect(p.power).toBe(1.5);
    expect(p.maxPixels).toBe(32);
  ...
  it('accepts all options', () => {
    ...
    expect(p.power).toBe(2.0);
    expect(p.maxPixels).toBe(16);
...
describe('GTAOPass.setRadius', () => {
  it('updates radius when positive', () => {
    const p = new GTAOPass();
    p.setRadius(1.5);
...
describe('GTAOPass apply lifecycle', () => {
...
describe('GTAOPass enabled=false', () => {
  it('does not render when disabled but still returns texture', () => {
    const gl = new MockGL2();
    const p = new GTAOPass({ enabled: false });

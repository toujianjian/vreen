...
// 覆盖:
//   1. 构造默认值与选项覆盖
//   2. setJitter 修改 jitter
//   3. getJitterPattern(Halton 序列)行为正确
//   4. apply() 在 mock GL 下不抛错并返回纹理
//   5. apply() 首帧分配内部纹理(3 张 RGBA16F)+ FBO + VAO
//   6. apply() 同尺寸不重复分配
//   7. apply() 多帧后 history 翻转(ping-pong)
//   8. dispose() 释放所有资源
//   9. dispose() 幂等
...
// ── MockGL2 ─────────────────────────────────────────────────────────
// 比 SSRPass.test 的 MockGL2 多 READ_FRAMEBUFFER / DRAW_FRAMEBUFFER 常量
// 与 blitFramebuffer 方法(TAA 用 blit 把 output → history)。
...
describe('TAAPass construction', () => {
  it('defaults: blendFactor=0.1, jitterScale=1.0', () => {
    const p = new TAAPass();
    expect(p.name).toBe('taa');
    expect(p.blendFactor).toBe(0.1);
    expect(p.jitterScale).toBe(1.0);
    expect(p.jitter.x).toBe(0);
    expect(p.jitter.y).toBe(0);
    expect(p.historyTexture).toBeNull();
    expect(p.velocityTexture).toBeNull();
  });

  it('accepts options', () => {
    const p = new TAAPass({ blendFactor: 0.2, jitterScale: 2.0 });
...
describe('TAAPass.setJitter', () => {
  it('updates jitter.x / jitter.y', () => {
    const p = new TAAPass();
    p.setJitter(0.25, -0.5);
    expect(p.jitter.x).toBe(0.25);
    expect(p.jitter.y).toBe(-0.5);
  });
});

// ── getJitterPattern (Halton) ──────────────────────────────────────

describe('TAAPass.getJitterPattern', () => {
  it('returns Vector2 in [-0.5, 0.5] * scale', () => {
    const p = new TAAPass({ jitterScale: 1.0 });
    for (let i = 0; i < 16; i++) {
      const j = p.getJitterPattern(i);
  ...
  it('sample 0 = (-0.5, -0.5) * scale (Halton(0, b) = 0)', () => {
    const p = new TAAPass({ jitterScale: 1.0 });
    const j = p.getJitterPattern(0);
    expect(j.x).toBeCloseTo(-0.5);
    expect(j.y).toBeCloseTo(-0.5);
  });

  it('sample 1 = Halton(1, 2)-0.5 = 0, Halton(1, 3)-0.5 = -1/6', () => {
    const p = new TAAPass({ jitterScale: 1.0 });
    const j = p.getJitterPattern(1);
    expect(j.x).toBeCloseTo(0.0); // Halton(1,2)=0.5 → 0
    expect(j.y).toBeCloseTo(-1 / 6); // Halton(1,3)=1/3 → -1/6
  });

  it('respects scale parameter', () => {
    const p = new TAAPass({ jitterScale: 1.0 });
    const j = p.getJitterPattern(0, 2.0);
    expect(j.x).toBeCloseTo(-1.0);
...
describe('TAAPass apply lifecycle', () => {
  it('apply() does not throw and returns a texture', () => {
    ...
    expect(gl.blitCalls).toBeGreaterThan(0); // blit 把 output → history
  });

  it('allocates 3 textures (output + historyA + historyB) + 3 FBOs + 1 VAO + 1 buf + 1 prog on first apply', () => {
    const gl = new MockGL2();
    const p = new TAAPass();
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('in'), makeTexture('v'));
  ...
  it('historyTexture is set after first apply', () => {
    const gl = new MockGL2();
    const p = new TAAPass();
    expect(p.historyTexture).toBeNull();
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('in'), makeTexture('v'));
    expect(p.historyTexture).not.toBeNull();
  });
...
describe('TAAPass dispose', () => {
  it('frees all 3 textures + 3 FBOs + VAO + buf + program after apply', () => {
    const gl = new MockGL2();
    const p = new TAAPass();
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('a'), makeTexture('v'));
  ...
  it('clears historyTexture reference after dispose', () => {
    const gl = new MockGL2();
    const p = new TAAPass();
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('a'), makeTexture('v'));
    expect(p.historyTexture).not.toBeNull();
  ...
  it('apply() after dispose re-allocates', () => {
    ...
    p.dispose(gl as unknown as WebGL2RenderingContext);
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('b'), makeTexture('v'));
    expect(gl.createdTextures.length).toBe(6); // 3 + 3 (重建后)
  });
});

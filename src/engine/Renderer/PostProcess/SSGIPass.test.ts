...
//   7. frame 计数每 apply 自增
//   8. dispose() 释放资源 + 幂等
//   9. SSGI_FRAG shader 源码校验

import { describe, it, expect } from 'vitest';
import { SSGIPass } from './SSGIPass';
import { Camera } from '../../Cameras/Camera';
import { PerspectiveCamera } from '../../Cameras/PerspectiveCamera';
import { SSGI_FRAG } from '../../Materials/shaders';

// ── MockGL2 ─────────────────────────────────────────────────────────
// 支持 SSGIPass.apply 所需的全部 GL 调用表面,包括 ShaderProgram 编译路径。
...
describe('SSGIPass construction', () => {
  it('defaults: maxSteps=32, thickness=0.5, resolution=0.5, strength=0.5, radius=0.5, numRays=8', () => {
    const p = new SSGIPass();
    expect(p.name).toBe('ssgi');
  ...
  it('accepts all options', () => {
    ...
    expect(p.strength).toBe(0.8);
    expect(p.radius).toBe(1.0);
    expect(p.numRays).toBe(4);
    expect(p.jitterScale).toBe(0.5);
  });
});

// ── setResolution ───────────────────────────────────────────────────

describe('SSGIPass.setResolution', () => {
  ...
  it('does not change value when same', () => {
    const p = new SSGIPass({ resolution: 0.5 });
    p.setResolution(0.5);
    expect(p.resolution).toBe(0.5);
  });
});
...
describe('SSGIPass apply lifecycle', () => {
  ...
  it('allocates 1 texture + 1 FBO + 1 VAO + 1 buffer on first apply', () => {
    ...
    p.apply(
      gl as unknown as WebGL2RenderingContext,
      makeTexture('in'), makeTexture('p'), makeTexture('n'), makeCamera(),
    );
...
describe('SSGIPass frame counter', () => {
...
describe('SSGIPass dispose', () => {
  ...
  it('dispose is idempotent (3x calls)', () => {
    const gl = new MockGL2();
    const p = new SSGIPass();
    p.dispose(gl as unknown as WebGL2RenderingContext);
    p.dispose(gl as unknown as WebGL2RenderingContext);
  ...
  it('apply() after dispose re-allocates resources', () => {
    const gl = new MockGL2();
    const p = new SSGIPass();
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('a'), makeTexture('p'), makeTexture('n'), makeCamera());
    p.dispose(gl as unknown as WebGL2RenderingContext);
...
// ── SSGI_FRAG shader 源码校验 ─────────────────────────────────────

describe('SSGI_FRAG shader source', () => {
  it('is GLSL ES 3.0', () => {
    expect(SSGI_FRAG).toContain('#version 300 es');
  });

  it('declares all required uniforms', () => {
    expect(SSGI_FRAG).toContain('u_colorMap');
    expect(SSGI_FRAG).toContain('u_positionMap');
    expect(SSGI_FRAG).toContain('u_normalMap');
    expect(SSGI_FRAG).toContain('u_projection');
    expect(SSGI_FRAG).toContain('u_view');
    expect(SSGI_FRAG).toContain('u_cameraPos');
    expect(SSGI_FRAG).toContain('u_screenSize');
    expect(SSGI_FRAG).toContain('u_maxSteps');
    expect(SSGI_FRAG).toContain('u_thickness');
    expect(SSGI_FRAG).toContain('u_strength');
    expect(SSGI_FRAG).toContain('u_radius');
    expect(SSGI_FRAG).toContain('u_frame');
    expect(SSGI_FRAG).toContain('u_numRays');
    expect(SSGI_FRAG).toContain('u_jitterScale');
  });

  it('uses cosine-weighted hemisphere sampling (asin(sqrt(xi)))', () => {
    expect(SSGI_FRAG).toContain('asin(sqrt(xi1))');
  });

  it('uses golden angle temporal rotation', () => {
    expect(SSGI_FRAG).toContain('goldenAngle');
    expect(SSGI_FRAG).toContain('2.39996323');
  });

  it('uses IGN (Interleaved Gradient Noise) for jitter', () => {
    expect(SSGI_FRAG).toContain('52.9829189');
    expect(SSGI_FRAG).toContain('0.06711056');
  });

  it('has MAX_RAYS cap of 8', () => {
    expect(SSGI_FRAG).toContain('MAX_RAYS 8');
  });

  it('implements view-space thickness hit test', () => {
    expect(SSGI_FRAG).toContain('hitTestVS');
    expect(SSGI_FRAG).toContain('viewDepth');
    expect(SSGI_FRAG).toContain('depthDiff > 0.0');
    expect(SSGI_FRAG).toContain('depthDiff < u_thickness');
  });

  it('applies edge fade for screen-border hits', () => {
    expect(SSGI_FRAG).toContain('edgeFade');
    expect(SSGI_FRAG).toContain('smoothstep(0.0, 0.1');
  });

  it('applies distance attenuation', () => {
    expect(SSGI_FRAG).toContain('distAtten');
    expect(SSGI_FRAG).toContain('1.0 / (1.0 + hitDist');
  });

  it('applies cosine weight (N·rayDir)', () => {
    expect(SSGI_FRAG).toContain('cosWeight');
    expect(SSGI_FRAG).toContain('dot(worldNormal, rayDir)');
  });

  it('normalizes by total weight', () => {
    expect(SSGI_FRAG).toContain('indirect / totalWeight');
  });

  it('outputs indirect irradiance × strength', () => {
    expect(SSGI_FRAG).toContain('indirectIrradiance * u_strength');
  });

  it('skips sky pixels (no normal)', () => {
    expect(SSGI_FRAG).toContain('length(worldNormal) < 0.01');
  });

  it('skips back-facing pixels', () => {
    expect(SSGI_FRAG).toContain('dot(viewDir, worldNormal) <= 0.0');
  });

  it('builds TBN from normal without tangent attribute', () => {
    expect(SSGI_FRAG).toContain('cross(up, worldNormal)');
    expect(SSGI_FRAG).toContain('cross(worldNormal, T)');
  });
});

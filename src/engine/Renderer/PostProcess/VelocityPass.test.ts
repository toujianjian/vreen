...
//   3. updateMatrices 滚动 prev / curr 矩阵
//   4. apply() 在 mock GL 下不抛错并返回纹理
//   5. apply() 首帧分配内部纹理 + FBO + VAO + buffer
//   6. apply() 同尺寸不重复分配
//   7. apply() resolution 变更后重新分配
//   8. dispose() 释放所有资源
//   9. dispose() 幂等
...
// ── MockGL2 ─────────────────────────────────────────────────────────
// 比 SSRPass.test 的 MockGL2 多 RG16F / RG 常量(Velocity 输出双通道浮点)。
...
describe('VelocityPass construction', () => {
  it('defaults: resolution=1.0', () => {
    const p = new VelocityPass();
    expect(p.name).toBe('velocity');
    expect(p.resolution).toBe(1.0);
    expect(p.prevViewProjection).toBeDefined();
    expect(p.currViewProjection).toBeDefined();
  });

  it('accepts options', () => {
    const p = new VelocityPass({ resolution: 0.5 });
...
describe('VelocityPass.setResolution', () => {
...
describe('VelocityPass.updateMatrices', () => {
  it('rolls curr → prev and computes new curr from camera', () => {
    const p = new VelocityPass();
    const cam = makeCamera();
    // 第一帧:prev 保持零矩阵(identity),curr ← projection * view
    const prevBefore = new Float32Array(p.prevViewProjection.elements);
    p.updateMatrices(cam);
    // prev 应等于原 curr(初始 identity)
    expect(new Float32Array(p.prevViewProjection.elements)).toEqual(prevBefore);
    // curr 应该被改变(非 identity,因为 perspective * view 不再是 identity)
    const currElements = new Float32Array(p.currViewProjection.elements);
    expect(currElements.some((v) => v !== 0 && v !== 1)).toBe(true);
  });

  it('second call: prev = first curr, curr recomputed', () => {
    const p = new VelocityPass();
    const cam = makeCamera();
    p.updateMatrices(cam);
    const firstCurr = new Float32Array(p.currViewProjection.elements);
    // 第二帧不动相机,curr 应该不变,prev 应等于第一帧的 curr
    p.updateMatrices(cam);
    expect(new Float32Array(p.prevViewProjection.elements)).toEqual(firstCurr);
  });
});

// ── apply lifecycle ────────────────────────────────────────────────

describe('VelocityPass apply lifecycle', () => {
  it('apply() does not throw and returns a texture', () => {
    const gl = new MockGL2();
    const p = new VelocityPass();
    p.updateMatrices(makeCamera());
    const out = p.apply(
  ...
  it('allocates 1 texture + 1 FBO + 1 VAO + 1 buffer + 1 program on first apply', () => {
    ...
    const p = new VelocityPass();
    p.updateMatrices(makeCamera());
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('p'), makeCamera());
  ...
  it('does not re-allocate on subsequent apply with same canvas size', () => {
    ...
    const p = new VelocityPass();
    p.updateMatrices(makeCamera());
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('a'), makeCamera());
    const texAfterFirst = gl.createdTextures.length;
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('b'), makeCamera());
  ...
  it('returns the same texture across apply() calls (no resize)', () => {
    ...
    const p = new VelocityPass();
    p.updateMatrices(makeCamera());
    const t1 = p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('a'), makeCamera());
    const t2 = p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('b'), makeCamera());
    expect(t1).toBe(t2);
  });
});

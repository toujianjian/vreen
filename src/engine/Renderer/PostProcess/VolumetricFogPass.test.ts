// VolumetricFogPass 单元测试。
//
// 覆盖:
//   1. 构造默认值与选项覆盖
//   2. setDensity / setColor 修改属性
...
class MockGL2 {
  ...
  deleteShader(s: WebGLShader | null): void { /* no-op, track if needed */ void s; }
...
describe('VolumetricFogPass construction', () => {
  it('defaults: density=0.05, fogStart=5, fogEnd=200, godRaysStrength=0.4', () => {
    ...
    expect(p.godRaysStrength).toBe(0.4);
    expect(p.fogColor.r).toBeCloseTo(0.5);
    expect(p.fogColor.g).toBeCloseTo(0.55);
    expect(p.fogColor.b).toBeCloseTo(0.6);
    expect(p.lightColor.r).toBeCloseTo(1.0);
    expect(p.lightDir.x).toBeCloseTo(-0.5);
    expect(p.lightDir.y).toBeCloseTo(-1.0);
    expect(p.lightDir.z).toBeCloseTo(-0.3);
  });

  it('accepts all options', () => {
    const fogColor = new Color(0.1, 0.2, 0.3);
    const lightColor = new Color(0.9, 0.8, 0.7);
    ...
    expect(p.density).toBe(0.08);
    expect(p.fogStart).toBe(10);
    expect(p.fogEnd).toBe(100);
    expect(p.godRaysStrength).toBe(0.7);
    expect(p.fogColor.r).toBe(0.1);
    expect(p.lightColor.r).toBe(0.9);
    expect(p.lightDir.x).toBe(0);
    expect(p.lightDir.y).toBe(-1);
  });

  it('fogColor / lightDir / lightColor accept provided instances (reference assignment, like VignettePass)', () => {
    const fc = new Color(0.1, 0.2, 0.3);
    const ld = new Vector3(1, 2, 3);
    const lc = new Color(0.9, 0.8, 0.7);
    const p = new VolumetricFogPass({ fogColor: fc, lightDir: ld, lightColor: lc });
    expect(p.fogColor).toBe(fc);
    expect(p.lightDir).toBe(ld);
    expect(p.lightColor).toBe(lc);
  });
});
...
describe('VolumetricFogPass setters', () => {
  it('setDensity updates density', () => {
    const p = new VolumetricFogPass();
    p.setDensity(0.15);
    expect(p.density).toBe(0.15);
  });

  it('setDensity clamps negative to 0', () => {
    const p = new VolumetricFogPass();
    p.setDensity(-1.0);
    expect(p.density).toBe(0);
  });

  it('setColor copies the input color', () => {
    const p = new VolumetricFogPass();
...
describe('VolumetricFogPass apply lifecycle', () => {
...
describe('VolumetricFogPass dispose', () => {
  it('frees all resources after apply', () => {
    const gl = new MockGL2();
    const p = new VolumetricFogPass();
    p.apply(gl as unknown as WebGL2RenderingContext, makeTexture('a'), makeTexture('d'), makeCamera());
